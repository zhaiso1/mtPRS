#' Simulate disease GWAS data for mtPRS analysis
#'
...
#' @details Simulate disease GWAS data for mtPRS analysis into 12 (2x3x2) different genetic architectures
...
#' }
#'
generate_dis_data <- function(structure="CS", sparseness="no", rho_DT, rho_T, rho_C, K=4, m=10000, pcausal=0.01, blocksize=100, samplesize=700, h2_base=0.5, h2_target=0.5){
  print("Step 1. Prepare correlation matrix")
  Omega <- simu_dis_cor(structure, rho_C, rho_T, rho_DT, K)
  Sigma <- h2_base/(m*pcausal)*Omega
  Sigma <- nearPD(Sigma)$mat %>% as.matrix()

  print("Step 2. Simulate true effect size")
  TRUESIZE <- simu_dis_truesize(m, blocksize, pcausal, Sigma, K, sparseness)

  print("Step 3. Simulate base cohort")
  se_pool <- fread("https://github.com/zhaiso1/mtPRS/blob/main/data/se_pool.csv?raw=true") %>% as.matrix()
  ...
  base <- simu_dis_base(TRUESIZE, se_pool, K, m)

  print("Step 4. Simulate target cohort")
  G <- fread("https://github.com/zhaiso1/mtPRS/blob/main/data/Geno.csv?raw=true") %>% as.matrix()
  G <- G[,index]; colnames(G) <- paste0("SNP",1:m)
  target <- simu_dis_target(TRUESIZE, G, m, samplesize, h2_target, K)

  print("Step 5. Output results")
  xx <- t(TRUESIZE)
  rownames(xx) <- paste0("SNP",1:m);colnames(xx) <- c(paste0("mu",1:K),"beta")
  re <- list(base=base, target=target, corr=Omega, truesize=xx)
  re
}

simu_dis_cor <- function(structure, rho_C, rho_T, rho_DT, K){
  ...
  corr12 <- matrix(rho_DT, ncol=1)
  corr21 <- matrix(rho_DT, nrow=1)
  corr22 <- 1
...
simu_dis_truesize <- function(m, blocksize, pcausal, Sigma, K, sparseness){
  nblock <- m/blocksize
  beta <- rep(0,m)
  while(abs(sum(beta != 0) - pcausal*m) > 0.1*pcausal*m){
    ...
    for (k in 1:nblock) {
      for (j in 1:blocksize) {
        label <- runif(1)
        if(label <= pi[k]){truesize <- rmvnorm(1, mean = rep(0,K+1), sigma = Sigma) %>% as.vector()}
        if(label > pi[k]){truesize <- rep(0,K+1)}

        if(sparseness == "half"){phi <- c(rbinom(K, 1, 0.5), 1); truesize <- phi*truesize}
        if(sparseness == "more"){phi <- c(rbinom(K, 1, 0.25), 1); truesize <- phi*truesize}
...
simu_dis_base <- function(TRUESIZE, se_pool, K, m){
  SumStat <- list()
  for (k in 1:K) {
...
simu_dis_target <- function(TRUESIZE, G, m, samplesize, h2_target, K){
  index <- sample(1:nrow(G), samplesize) %>% sort()
  G_sub <- G[index,]
  nsubj <- nrow(G_sub)
  beta <- TRUESIZE[K+1,]
  main_part <- (G_sub%*%beta) %>% as.matrix() %>% as.vector()
  sigma <- ((1-h2_target)/h2_target*var(main_part)) %>% sqrt()
  Y <- main_part + rnorm(nsubj, mean = 0, sd=sigma)

  ind <- cbind.data.frame(Y=Y, G_sub)
  return(ind)
}
